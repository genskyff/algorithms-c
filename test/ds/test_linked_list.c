...
void test_init(void) {
    ...
    assert(list.head != NULL, msg);
    assert_eq(length(&list), LEN, msg);
    assert_arr_eq(arr, length(&list), tmp, LEN, msg);
...
void test_from_array(void) {
    ...
    assert(from_array(NULL, LEN).head == NULL, msg);
}
...
void test_swap(void) {
    ...
    assert_arr_eq(tmp, length(&list), swap_0_5, LEN, msg);
    free(tmp);

    msg = "should swap(0, 1)";
    swap(&list, 0, 1);
    tmp = to_array(&list);
    assert_arr_eq(tmp, length(&list), swap_0_1, LEN, msg);
    free(tmp);
...
void test_reverse(void) {
    ...
    reverse(&list);
    elem_t *tmp = to_array(&list);
    assert_arr_eq(tmp, length(&list), tmp, LEN, msg);
    free(tmp);
}
...
void test_get(void) {
    ...
    assert(!get(&list, length(&list), &e), msg);
}
...
void test_set(void) {
    ...
    assert(!set(&list, length(&list), e), msg);
}
...
void test_insert(void) {
    ...
    assert_eq(list.head->data, 10, msg);
    assert_eq(length(&list), LEN + 1, msg);

    msg = "should not insert when out of range";
    assert(!insert(&list, length(&list) + 1, 10), msg);
}
...
void test_del(void) {
    ...
    assert(!del(&list, length(&list), &e), msg);
}
